---
layout: post
title: 《Java Concurrency In Practice》阅读笔记
...
**更新**：这章节内部的叙事怎么能这么乱，同一个事情拆到几个小节分散着讲，车轱辘话来回说。

**更新**：第 8 章开始比较轻松了，都是熟悉的内容，不过书里面也讲到了以前没考虑的事情，也有新的收获。

**更新**：应该不会再往下写了，以自己的背景和履历，终究还是入不了行
...
public class PrimeGenerator implements Runnable {
    ...
    // 使用 volatile 保证内存可见性，这个 flag 被别的线程设置为 true 时，取消当前任务的执行
...
包括 Java 在内的所有基于共享内存模型的语言都没有提供强制终止线程的方法，而是提供了一种交互式的终止方式，让一个线程中断另一个线程，表达希望另一个线程终止的意图，被中断的线程捕获 `InterruptedException` 响应中断，执行任务取消逻辑。这是在 Java 中取消任务的最佳实践。

Java 中的大部分阻塞方法一般都会抛出 `InterruptedException`，目的就是为了提供提前取消任务，从阻塞状态退出的机制，防止无限阻塞导致的线程活跃性问题。虽然 JVM 不保证这些阻塞方法会在多长时间内检测到中断，但是实践中，这个时间一般是很短的。

对于执行不包含阻塞方法的任务的线程来说，中断它不会导致抛出 `InterruptedException`，而是只会设置一个中断标志，开发者需要自行在任务逻辑里面通过 `isInterrupted()` 方法检测中断标志，执行对应的任务取消逻辑。
...
public class Thread {
    ...
    // 检查当前线程的中断状态，这是主动清除当前线程中断标志的唯一手段，需要谨慎使用
...
class PrimeProducer extends Thread {
    ...
    public void run() {
        try {
            ...
            // 此处定义了任务的取消策略，一般有两种策略：
            // 1. 马上终止任务，执行回滚
            // 2. 延迟终止任务，先把当前工作执行完
            Thread.currentThread().interrupt(); // 最后要恢复中断标志位或者重新抛出异常
        }
    }
...
就像任务有取消策略一样，线程有中断策略，这两个概念要明确区分：

- 任务的取消策略：定义了任务怎么取消执行
- 线程的中断策略：定义了线程怎么响应中断

此处非常固有迷惑性的一点是，任务的取消策略是定义在处理中断异常的代码块中的，从而误认为线程的中断策略就是任务的取消策略。

消除这个迷惑的关键是要认识到，任务本身并不拥有执行它的线程，而是线程池拥有执行任务的线程，并把中断策略封装了起来。

```java
// === 这是线程拥有者（线程池）的伪代码范围 ===
// Thread Policy (线程的中断策略) 体现在这里
    ...
    try {
        ...
        while (task != null || (task = getTask()) != null) { 
            try {
                ...
                // 线程策略的一部分：如果任务抛出异常，线程该怎么办？
                // 可能会记录日志，或者让这个线程死掉重启一个新的
                handleException(x); 
            }
            
            // 关键点：如果 task.run() 吞掉了中断（即没有恢复中断状态）
            // 这里的代码就不知道刚才发生过中断，
            // 可能会错误地继续去 getTask() 等待下一个任务，导致线程无法响应 shutdown。
        }
    } finally {
...
如果能理解这一点，那么就会理解这样的事实，对于不由线程池管理的部分线程。比如直接 `new Thread(task).start()` 创建的线程，它的中断策略和任务的取消策略在物理上是一致的，但在逻辑上，仍然应该认识到这两个概念是分离的。
...
public static void timedRun(final Runnable r, long timeout, TimeUnit unit) throws InterruptedException { 		class RethrowableTask implements Runnable {
    ...
    // 由于 join 返回后，并不能知道任务是否执行成功，所以这里要重新抛出任务执行中可能出现的异常
	// 这被认为是 Java API 的一个设计缺陷
    task.rethrow();
}
...
public static void timedRun(Runnable r, long timeout, TimeUnit unit) throws InterruptedException { 
    ...
    try {
        ...
        // 抛出这个异常并不意味着任务执行停止了，此时任务仍然还在运行
		// task will be cancelled below 
    } catch (ExecutionException e) {
        // 抛出这个异常时，可以认为任务已经停止执行了
...
比如，如果要设计一个使用了阻塞方法，但又不想支持取消机制的任务，那么此时可以考虑暂时吞掉 `InterruptedException`，后续在合适的时机恢复中断。通常的做法是在死循环中调用阻塞方法，但是一定要注意，在死循环中检测到中断的时候不要立刻恢复中断，而是记录被中断过的事实后，直接进入下一轮循环重试任务，等任务执行成功后再恢复中断。如果直接恢复中断，那么后面每一轮循环时，阻塞方法都会直接抛出 `InterruptedException`，如此往复，陷入死循环。
...
即使你的任务没有调用阻塞方法，不断轮询当前线程的中断状态也可以达到更好的响应速度，但也这会增加 CPU 资源的消耗，在一些对响应速度要求比较高的任务中可以考虑这么做。

Java 里面还有小部分阻塞方法是不响应中断的，比如 Socket 的同步 IO 还有内部锁的获取，对于这些方法来说，中断只会设置线程中断标志，不会抛出 `InterruptedException`，无法提前从阻塞状态退出，对于这些方法，需要使用一些粗暴的手段：

- 对于 `java.io` 的同步 Socket IO，直接关闭 Socket，让这些 IO 抛出 `SocketException`
- 对于 `Selector` 提供的异步 IO，调用 `wakeup()` 让 `select()` 抛出 `ClosedSelectorException`
- 对于阻塞在内部锁获取上的线程，没有好的办法，除非最开始就不使用内部锁
...
public class ReaderThread extends Thread {
    ...
    public void interrupt() {
        try {
            // 这部分保证阻塞在 Socket 同步 IO 上时可以提前取消
            socket.close();
        } catch (IOException ignored) {
            // ignored
        } finally {
            // 这部分保证阻塞在其他可以响应中断的阻塞方法上时可以提前取消
            super.interrupt();
        }
    }
...
/*
 * 给 Callable 增加用于取消执行的 cancel() 接口和创建 RunnableFuture 的接口
 */
public interface CancellableTask<T> extends Callable<T> {
    void cancel();
    RunnableFuture<T> newTask();
...
/**
 * 重写 newTaskFor() 添加自定义的 RunnableFuture
 */
public class CancellingExecutor extends ThreadPoolExecutor {
    // ...
    protected <T> RunnableFuture<T> newTaskFor(Callable<T> callable) {
...
JVM 有两种关闭方式：有序关闭和强制关闭。

有序关闭时，JVM 首先会启动所有停机钩子（Shutdown Hook），停机钩子是指使用 `Runtime.addShutdownHook` 注册的，尚未启动的线程，JVM 不保证这些线程的执行顺序，它们是与业务线程一起并发执行的，所有的停机钩子执行结束后，如果 `runFinalizersOnExit` 被设置位 `true` ，JVM 会执行终结器（Finalizer），最后，停止 JVM 的运行，业务线程会被强制关闭。

> 应用开发者应该极力避免使用终结器。
{: .prompt-warning }

如果是强制关闭，那 JVM 不会做停止运行之外的任何事情。

停机钩子一般用来执行服务和资源的清理，比如删除临时文件，关闭日志文件等。
...
如果停机钩子执行时间过长，JVM 关闭所需的时间也会被拉长，所以在设计停机钩子时，需要保持代码的执行效率；另外，由于停机钩子是和业务线程并发运行的，所以也需要特别注意死锁问题，编写代码时，不要对应用程序的状态做出任何假设，进行防御性编程。

最佳实践是，不要为每个服务都单独注册一个停机钩子，而是把所有服务的终止逻辑都注册在同一个停机钩子里面，这样可以有效减少死锁发生的概率，额外的好处是，如果服务之间有依赖性，这样可以根据依赖关系有序停止服务。

JVM 启动时，除了会启动 Main 线程这个非守护线程之外，还会启动包括 GC 线程在内的多个维护性线程。

守护线程和非守护线程的区别只有一个：当所有非守护线程都停止运行后，JVM 会自己有序关闭，不管此时守护线程是否还在运行。所以，守护线程中不要执行不能随时终止的逻辑，尤其是跟 IO 相关的逻辑。最好只执行一些维护性事务，比如检查过期缓存之类的。
...
虽然之前提到线程池可以把任务的提交和执行分离，即可以在不改变提交逻辑的情况下，替换 `Executor` 的实现切换不同的执行策略，但这种说法其实有些夸张。

线程池在处理大量同质化，且各自独立的任务时是非常高效的。

但有一部分任务是不能同时兼容所有执行策略的，这些任务需要配合特定的执行策略，换言之，这些任务对线程池的执行策略是做了一定假设的。

- 有执行结果的依赖关系的任务

  如果任务 B 依赖任务 A 的执行结果，但此时所有的线程都在被任务 B 占用，等待任务 A 的执行结果，此时会导致死锁发生。

  这种任务假设线程池可以无限创建新的线程，这样可以保证总是有额外的线程可以执行其中一个任务，避免循环等待。

- 需要线程封闭的任务

  除了单线程线程池之外的其他线程会破坏线程封闭。

  这种任务假设线程池的单线程的，这样可以避免共享变量被其他线程访问到，降低额外的同步成本。

- 响应时间敏感的任务

  线程池会给任务引入额外的排队时间，这个时间是不可控的，尤其是长短任务混合提交的情况下。

  这种任务假设线程池会马上处理新任务，没有排队等待时间。

  > 编写这类时间敏感的任务时，如果需要用到阻塞方法，那么使用带超时时间的重载版本会好些。
  >
  {: .prompt-tip}

- 使用了 `ThreadLocal` 的任务

  线程池的线程复用会导致数据污染。任务可能会读取到上一个线程遗留的 `ThreadLocal` 旧值。

  这种任务假设线程池总是使用新线程执行新任务，而不是复用现有线程。

### 调整线程池大小

线程池的大小取决于要执行的任务的类型和部署的系统的性质。但不论怎么说，线程池的大小不应该被硬编码进代码里面，应该用一种可配置的方式提供或者基于 `Runtime.availableProcessors` 动态计算。

具体配置多大的线程池不是很好确定，也不需要确定到那么精准，只要避免过大或者过小就可以了。

过大的线程池会导致软硬件资源争用太激烈，导致实际执行任务的资源变少，过小的线程数可能会导致资源利用率不足，无法达到最佳的吞吐量。

为了确定合理的线程池大小，我们需要考虑这些问题：

- 应用将要部署的环境有多少 CPU 核心、多少内存
- 任务是 IO 密集型还是 CPU 密集型，还是二者混合
- 任务是否需要用到某些稀缺资源，比如 JDBC 连接

根据这些问题答案的不同，我们可能需要选择多个不同的线程池分别处理对应性质的任务。

为了使处理器保持在预期的利用率，最佳线程池大小（Optimal pool size）的计算公式为：

$$N_{threads} = N_{cpu} * U_{cpu} * \left(1 + \frac{W}{C}\right)$$

其中：

- $N_{cpu}$ = CPU 核心数 (number of CPUs)
- $U_{cpu}$ = 目标 CPU 利用率 (target CPU utilization)，$0 \leq U_{cpu} \leq 1$
- $\frac{W}{C}$ = 等待时间与计算时间的比例 (ratio of wait time to compute time)

可以通过 `Runtime.getRuntime().availableProcessors();` 动态获取系统的 CPU 核心数。

为了正确设置线程池的大小，必须估算任务的等待时间与计算时间的比例；这个估算不一定需要非常精确，可以通过性能分析来获得。或者，可以通过在基准负载下使用不同的线程池大小运行应用程序，并观察 CPU 利用率来调整线程池的大小，这也是实践中最常用的办法。

当然，CPU 时间并不是需要通过线程池来管理的唯一资源。其他可能影响资源分配限制的资源还包括内存、文件句柄、套接字句柄以及数据库连接。

对于这些类型的资源，线程池大小的限制相对较为简单：只需用可用的总资源量除以将每个任务所需的资源量即可。所得结果将作为池大小的上限值。 

当任务需要共享资源（例如数据库连接）时，线程池大小和资源池大小会相互影响。如果每个任务都需要一个连接，那么线程池的有效大小将受到连接池大小的限制。同样地，如果连接的唯一使用者是池任务，那么连接池的有效大小将受到线程池大小的限制。

### 配置 ThreadPoolExecutor

ThreadPoolExecutor 是 Executor 框架的基础实现，提供了一个灵活的构造方法：
...
- `corePoolSize` – 线程池中保持的线程数量，即使线程是空闲的也会保留，除非设置 `allowCoreThreadTimeOut` 为 `true`

  > 当创建 ThreadPoolExecutor 时，核心线程不会立即启动，而是会在接收到任务请求时才启动，除非你调用了 `prestartAllCoreThreads` 方法。
  {: .prompt-tip }

- `maximumPoolSize` – 线程池中允许的最大线程数量

- `keepAliveTime` – 非核心线程保持空闲的允许的最长时间，超出这个时间，线程会终止

- `unit` – `keepAliveTime` 的时间时间单位

- `workQueue` – 保存待执行任务的阻塞队列

- `threadFactory` – 用于创建新线程的线程工厂

- `handler` – 当线程容量限制和队列容量限制都已达到时，对新任务的处理策略

`Executors` 提供的那些默认实现都是基于这个构造方法构造的。

- `newFixedThreadPool` 的核心线程数和最大线程数都一样，这样可以实现线程的永不销毁
- `newCachedThreadPool` 的核心线程数是 0，最大线程数是 Integer.MAX_VALUE，超时时间 1 分钟，这样可以实现一个无限扩张的线程池，然后在空闲时又可以回缩。

​	......

开发者有时会倾向于将核心线程数设置为 0，以便工作线程最终能被销毁，从而不会阻止 JVM 退出。但是，对于那些没有像 `newCachedThreadPool` 那样使用 `SynchronousQueue` 作为工作队列的线程池来说，这种做法会导致一些看似奇怪的行为。

如果线程池的规模已经达到了核心线程数，`ThreadPoolExecutor` 只有在工作队列已满的情况下才会创建新线程。因此，如果将一个具有一定容量的工作队列与核心线程数为 0 的配置结合使用，提交给该线程池的任务在队列填满之前都不会执行，而这通常不是开发者想要的结果。

不过 `allowCoreThreadTimeOut` 允许你请求让所有池内线程都能超时销毁；如果你想要一个既有界（包括线程池大小和工作队列）又能在无活可干时销毁所有线程的线程池，可以设置一个正常的核心线程数然后开启 `allowCoreThreadTimeOut`，以此来达到核心线程数为 0 的效果。
